...
func (s *ConfigStorage) Update(ctx context.Context, req domain.ConfigUpdateRequest, requestID string) ([]domain.ConfigItem, error) {
...
func parseVersionConflict(err error, entries []domain.ConfigUpdateEntry) error {
	if err == nil {
		return nil
	}
	message := err.Error()
